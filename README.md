...
| `copilotCompact.responsesCompactModel` | `""` | Optional explicit model sent to `/responses/compact`, e.g. `gpt-5.5`. Keep it empty for generic per-model matching. Empty means auto-fetch `/models`, match a compact channel, and send the base model. |
| `copilotCompact.compactModelOverride` | `""` | Deprecated alias of `responsesCompactModel`. |
...
| 配置项 | 默认值 | 说明 |
| --- | --- | --- |
| `copilotCompact.enabled` | `true` | 启用或禁用请求改写。 |
| `copilotCompact.targetHosts` | `[]` | 可选目标域名白名单；为空表示匹配所有域名。示例：`["newapi.example.com"]`。 |
| `copilotCompact.compactPathSuffix` | `"compact"` | 追加到 `/responses` 后的路径片段。 |
| `copilotCompact.responsesCompactModel` | `""` | 可选的 compact 请求模型，例如 `gpt-5.5`；如果你会切换多个模型，建议保持为空以启用通用自动匹配。为空时自动请求 `/models`，匹配 compact 通道后发送基础模型。 |
| `copilotCompact.compactModelOverride` | `""` | `responsesCompactModel` 的兼容别名，后续不建议使用。 |
| `copilotCompact.rewriteMode` | `"compactOnly"` | `compactOnly` 只改写识别到的压缩请求；`allResponses` 会改写所有 `POST /responses` 请求，仅建议调试时使用。 |
| `copilotCompact.logLevel` | `"info"` | 可选 `off`、`info` 或 `debug`。 |
